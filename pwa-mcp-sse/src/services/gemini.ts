...
export const callGemini = async (prompt: string, tools: any[], resources: any[] = []) => {
  ...
  try {
    ...
    if (functionCalls && functionCalls.length > 0) {
      ...
      // Execute each tool call via SSE HTTP requests
      const toolResults = [];
      for (const functionCall of functionCalls) {
        try {
          ...
          if (functionCall.functionCall.name === "read_animal_info") {
            ...
            toolResponse = await sseService.readResource(resourceUri);
            ...
            console.log('Calling tool via SSE:', functionCall.functionCall.name);
            
            toolResponse = await sseService.callTool(
              functionCall.functionCall.name,
              functionCall.functionCall.args || functionCall.functionCall.arguments
            );
          ...
          for (const functionCall of newFunctionCalls) {
            try {
              ...
              if (functionCall.functionCall.name === "read_animal_info") {
                ...
                // Handle regular tool calls (including get_coordinates)
                toolResponse = await sseService.callTool(
                  functionCall.functionCall.name,
                  functionCall.functionCall.args || functionCall.functionCall.arguments
                );
